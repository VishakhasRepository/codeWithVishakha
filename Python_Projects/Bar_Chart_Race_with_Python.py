...
df = df.loc[df['Variable'] == 'GDP per capita in USA 2005 PPPs']  #filtering the csv file
...
#df.to_csv("gdp per capita_out1.csv")
df = df.pivot(index='Country', columns='Time', values='Value')
#df.to_csv("gdp per capita_out2.csv")
df = df.reset_index()
#df.to_csv("gdp per capita_out3.csv")
